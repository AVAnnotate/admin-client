...
import type { apiAddTag, apiUpdateTag, apiDeleteTag } from '@ty/api.ts';
import { userInfo } from '@backend/userInfo.ts';
import { parseSlug } from '@backend/projectHelpers.ts';
import type { ProjectData, UserInfo } from '@ty/Types.ts';
import { addTag, updateTag, deleteTag } from '@backend/tagHelpers.ts';
...
export const DELETE: APIRoute = async ({
  ...
  if (request.headers.get('Content-Type') === 'application/json') {
    ...
    const body: apiDeleteTag = await request.json();

    const project: ProjectData | undefined = await deleteTag(
      repoUrl,
      info as UserInfo,
      body.tag
    );
    ...
    if (project) {
      ...
      return new Response(null, {
        status: 500,
        statusText: 'Failed to delete Tag: ' + body.tag,
      });
    }
  }
  return new Response(null, {
